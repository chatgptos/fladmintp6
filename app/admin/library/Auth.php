...
class Auth extends \fast\Auth
{
    ...
    public function login($username, $password, $keeptime = 0)
    {
        ...
        if (Config::get('fladmin.login_failure_retry') && $admin->loginfailure >= 10 && time() - $admin->updatetime < 86400) {
    ...
    public function isLogin()
    {
        ...
        if (Config::get('fladmin.login_unique')) {
        ...
        //判断管理员IP是否变动
        if (Config::get('fladmin.loginip_check')) {
    ...
    public function getSidebar($params = [], $fixedPage = 'dashboard')
    {
        ...
        $menu = $nav = '';
        if (Config::get('fladmin.multiplenav')) {
            $topList = [];
            foreach ($ruleList as $index => $item) {
                if (! $item['pid']) {
                    $topList[] = $item;
